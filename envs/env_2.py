...
class test_env():
    ...
    def calculate_reward(self, config=CONFIG):
        ...
        if self.pos_agent == self.pos_target:
            ...
            self.reward = 1  # (27 - self.min_steps())**config['multiplier'] #1/abs(self.get_observation())
        else:
            self.reward = -1  # -(27 - self.min_steps())**config['multiplier']#-self.min_steps()*config['multiplier']#-abs(self.get_observation()*config['multiplier'])
